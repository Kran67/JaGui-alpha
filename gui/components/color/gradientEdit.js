(function(){
var GradientEdit=$j.classes.GraphicControl.extend({
    _ClassName:"GradientEdit",
    ...
    getChildsDOMObj:function() {
      ...
      for (var i=0,l=childs.length;i<l;i++) {
        if (childs[i].nodeType===$j.types.xmlNodeTypes.ELEMENT_NODE) {
          ...
          grad._DOMObj=childs[i];
          grad._DOMObj.jsObj=this;
          grad._DOMObj.gradItem=grad;
          $j.tools.events.bind(grad._DOMObj,$j.types.mouseEvents.DOWN, this.selectPoint);
          $j.tools.events.bind(grad._DOMObj,$j.types.mouseEvents.MOVE, this.movePoint);
    ...
    selectPoint:function(event) {
      ...
      if ($j.mouse.button==$j.types.mouseButtons.LEFT) {
        if (grdEdit._currentPoint!==-1) {
          if ($j.browser.ie) grdEdit.gradient.items[grdEdit._currentPoint]._DOMObj.setAttribute("data-selected",false);
          else grdEdit.gradient.items[grdEdit._currentPoint]._DOMObj.dataset.selected=false;
        }
    ...
    removePoint:function(gradItem) {
      $j.tools.events.unBind(gradItem._DOMObj,$j.types.mouseEvents.DOWN, this.selectPoint);
      $j.tools.events.unBind(gradItem._DOMObj,$j.types.mouseEvents.MOVE, this.movePoint);
      gradItem._DOMObj.jsObj=null;
      this._colorStopsArea.removeChild(gradItem._DOMObj);
      this.gradient.items.removeAt(this.gradient.items.indexOf(gradItem));
      var data=($j.browser.ie)?gradItem._DOMObj.getAttribute("data-selected"):gradItem._DOMObj.dataset.selected;
      if (data==="true") {
        this._currentPoint=this._currentPoint-1;
        if (this._currentPoint<0) this._currentPoint=0;
        if ($j.browser.ie) this.gradient.items[this._currentPoint]._DOMObj.setAttribute("data-selected",true);
        else this.gradient.items[this._currentPoint]._DOMObj.dataset.selected=true;
      }
      //this.doChanged(this);
      gradItem._DOMObj=null;
      this.update();
    },
    ...
    mouseMove:function() {
      if ($j.mouse.button===$j.types.mouseButtons.LEFT) {
        if (this._currentPoint>-1) {
          console.log("ok");
          var gradDom=this.gradient.items[this._currentPoint]._DOMObj;
    ...
    mouseDown:function() {
      if ($j.mouse.button===$j.types.mouseButtons.LEFT) {
        if (this._currentPoint!==-1) {
          if ($j.browser.ie) this.gradient.items[this._currentPoint]._DOMObj.setAttribute("data-selected",false);
          else this.gradient.items[this._currentPoint]._DOMObj.dataset.selected=false;
        }
        this._currentPoint=-1;
      }
    },
    ...
    changeCurrentPointColor:function(color) {
      if (this.currentPoint!==-1) {
        ...
        this.gradient.items[this._currentPoint]._DOMObj.style[$j.types.jsCSSProperties.BACKGROUNDCOLOR]=color.toARGBString();
        this.update();
      }
    }
    //#endregion

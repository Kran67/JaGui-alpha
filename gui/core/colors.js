(function() {
  //#region Color
  $j.classes.Color=$j.classes.Bindable.extend({
    _ClassName: "Color",
    init: function() {
      this.red=0;
      this.green=0;
    ...
    setRed:function(newValue) {
      ...
      if (newValue!==this.red) {
        ...
        if (!$j.tools.isNull(this._owner)&&!this._owner._loading) this._owner.update();
      }
    },
    ...
    setHSL:function setHSL(hue,saturation,lightness) {
      if (typeof hue!==_const.NUMBER) return;
      if (typeof saturation!==_const.NUMBER) return;
    ...
    //#endregion
    //#region Methods
    clone: function Color_clone(){
      var c=new $j.classes.Color(this.red,this.green,this.blue,this.alpha);
      return c;
    },
    toRGBHexString: function Color_toRGBHexString(){return ("#"+_conv.dec2Hex(this.red).padLeft(2,'0')+_conv.dec2Hex(this.green).padLeft(2,'0')+_conv.dec2Hex(this.blue).padLeft(2,'0')).toUpperCase();},
    toARGBHexString: function Color_toARGBHexString(){return ("#"+_conv.dec2Hex(this.alpha*0xFF).padLeft(2,'0')+_conv.dec2Hex(this.red).padLeft(2,'0')+_conv.dec2Hex(this.green).padLeft(2,'0')+_conv.dec2Hex(this.blue).padLeft(2,'0')).toUpperCase();},
    toRGBString: function Color_toRGBString(){return "rgb("+this.red+","+this.green+","+this.blue+")";},
    toARGBString: function Color_toARGBString(){return "rgba("+this.red+","+this.green+","+this.blue+","+this.alpha+")";},
    toHSLString: function Color_toHSLString(){return "hsl("+this.hue+","+this.saturation+","+this.lightness+")";},
    toHSVString: function Color_toHSVString(){return "hsv("+this.hue+","+this.saturation+","+this.value+")";},
    append: function Color_append(color){
    ...
    subtract: function Color_subtract(color){
      if (!(color instanceof $j.classes.Color)) return;
      if (this.alpha-color.alpha<1) this.alpha=color.alpha-s1.alpha;
      else this.alpha=1;
    ...
    RGBtoBGR: function Color_RGBtoBGR(){
      this.red=this.blue;
      this.blue=this.red;
      return this;
    },
    premultyAlpha: function Color_premultyAlpha(){
      if (this.alpha===0) {
        this.red=0;
        this.green=0;
        this.blue=0;
    ...
    unPremultyAlpha: function Color_unPremultyAlpha(){
      if (this.alpha===0) {
        this.red=0;
        this.green=0;
        this.blue=0;
    ...
    opacity: function Color_opacity(opacity){
    ...
    equals: function Color_equals(color){
    ...
    assign: function Color_assign(source) {
    ...
    endUpdate: function() {
	  ...
	  HSVtoRGB: function Color_HSVtoRGB() {
	  ...
	  HSLtoRGB: function Color_HSLtoRGB() {
		  var sat=this.saturation/100,
	  ...
	  RGBtoHSV: function Color_RGBtoHSV() {
	  ...
	  RGBtoHSL: function Color_RGBtoHSL() {
		  var red=this.red/255,
		      green=this.green/255,
		      blue=this.blue/255,
          cMax=$j.max(red,green,blue),
  ...
  // Public
  $j.tools.colors={
    createFromRGBA: function Color_createFromRGBA(red,green,blue,alpha) {
    ...
    createFromHSL: function Color_createFromHSL(hue,saturation,lightness) {
      var c=new $j.classes.Color;
    ...
    createFromHSV: function Color_createFromHSV(hue,saturation,value) {
      var c=new $j.classes.Color;
      if (typeof hue!==_const.NUMBER) hue=0;
      if (typeof saturation!==_const.NUMBER) saturation=0;
    ...
    newColor: function Colors_newColor(red,green,blue,alpha) {
      return new $j.classes.Color(red,green,blue,alpha);
    },
    parse: function Colors_parse(strColor) {
      var color_defs=[
        {
    ...
    changeHSL: function Colors_changeHSL(c,h,s,l){

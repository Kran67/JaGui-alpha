...
(function() {
  ...
  if(!String.prototype.firstCharUpper) {
    String.prototype.firstCharUpper=function() {
      return [this.charAt(0).toUpperCase(),this.substr(1).toLowerCase()].join("");
    };
  }
  // Méthode compareTo ->
  // Retourne la nouvelle chaine
  if(!String.prototype.compareTo) {
    String.prototype.compareTo=function(s) {
      ...
      for (var i=0,n=$j.max(a.length,b.length);i<n&&a.charAt(i)===b.charAt(i);++i);
      if (i===n) return 0;
  ...
  if(!String.prototype.isBoolean) {
    String.prototype.isBoolean=function() { return (!$j.tools.isNull(this)&&("true"===this.trim()||"false"===this.trim())); };
  }
  ...
  if(!String.prototype.format) {
    $j.tools.formatSpecifier=function(s) {
	  ...
	  $j.tools.pad=function(s,flag,len) {
      var c,rslt;
    ...
    String.prototype.format=function() {
      ...
      for(var i=0;i<sf.length;i++) {
        ...
        if(s==="%%") {
          ...
          frmt=new $j.tools.formatSpecifier(s);//get the formating object
          ...
          if(frmt.type==="s") {//String
            ...
            s=obj.toString();
            s=$j.tools.pad(s,frmt.paddingFlag,frmt.minLength);
            ...
            switch(frmt.type) {
              ...
              case "b": //binary
                s=obj.toString(2);
                s=$j.tools.pad(s,"0",frmt.percision);
                break;
              case "o": //octal
                s=obj.toString(8);
                s=$j.tools.pad(s,"0",frmt.percision);
            ...
            if(frmt.paddingFlag==="0") {//do 0-padding
              ...
              s=$j.tools.pad(s,"0",frmt.minLength-sign.length);
            }
            s=sign+s;//add sign
            s=$j.tools.pad(s,frmt.paddingFlag,frmt.minLength);//do padding and justifiing
          } else {
  ...
  String.uuid=(function() {
    ...
    return function(l,r) {
      ...
      if(l) {
        // Compact form
        for(i=0;i<l;i++) uuid[i]=chars[0||$j.random()*r];
      } else {
